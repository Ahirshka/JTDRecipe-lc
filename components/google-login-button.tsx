...
import { getGoogleAuthUrl, initializeGoogleAuth } from "@/lib/google-auth"
...
export function GoogleLoginButton({ disabled, className }: GoogleLoginButtonProps) {
  ...
  return (
    <Button
      variant="outline"
      className={`w-full relative overflow-hidden border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 ${className}`}
    ...
    >
      <div className="flex items-center justify-center w-full">
        ...
        <span className="font-medium">{isLoading ? "Connecting to Google..." : "Continue with Google"}</span>
      </div>
    </Button>
  )
}
